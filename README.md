...
ConcernChoiceWhyBidsstd::map<uint64_t, std::deque<Order>, std::greater<uint64_t>>Ordered by price descending → the best (highest) bid is always begin()Asksstd::map<uint64_t, std::deque<Order>>Ordered by price ascending → the best (lowest) ask is always begin()Orders at one pricestd::deque<Order>FIFO queue → preserves time priority, with O(1) push-back / pop-frontTrade logstd::vector<Trade>Append-only record of every fill, exported to CSV
